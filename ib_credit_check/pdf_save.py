...
def screen_print(readme):
    ...
    for i in firm_list:
        ...
        pdfkit.from_url(url_yingji, i+'-应急管理部.pdf', options = options)
        pdfkit.from_url(url_tax, i+'-税务总局.pdf', options = options)
        pdfkit.from_url(url_guotu, i+'-自然资源部.pdf', options = options)
        pdfkit.from_url(url_haiguan, i+'-海关.pdf', options = options)
        pdfkit.from_url(url_env, i+'-环境部.pdf', options = options)
        try:
